...
// Helper function to select a targeted affirmation based on mood
// This function should be outside the component to avoid re-creation on every render
const selectTargetedAffirmation = (mood: number): Affirmation => {
  ...
  if (mood <= 2) {
    ...
    // For neutral moods, focus on strength or hope
    category = Math.random() > 0.5 ? 'strength' : 'hope';
  } else {
    // For positive moods, focus on healing or strength
    category = Math.random() > 0.5 ? 'healing' : 'strength';
  }

  const filteredAffirmations = affirmations.filter(a => a.category === category);
  // Fallback if no affirmations match the category (though unlikely with current data)
  if (filteredAffirmations.length === 0) {
    return { text: "Your feelings are valid. Be gentle with yourself today.", category: 'healing' };
  }
  const randomIndex = Math.floor(Math.random() * filteredAffirmations.length);
  return filteredAffirmations[randomIndex];
};
...
const MoodTrackerPage: React.FC = () => {
  ...
  const [moodNote, setMoodNote] = useState('');
  const [dailyAffirmation, setDailyAffirmation] = useState<Affirmation | null>(null);
  // New state to hold the affirmation dynamically selected based on currentMood
  const [targetedMoodAffirmation, setTargetedMoodAffirmation] = useState<string | null>(null);

  useEffect(() => {
    // Set a random affirmation when component mounts (this is the general 'Daily Affirmation')
    const randomIndex = Math.floor(Math.random() * affirmations.length);
    setDailyAffirmation(affirmations[randomIndex]);
  }, []);

  // New useEffect to set targeted affirmation only when currentMood changes
  useEffect(() => {
    if (currentMood !== null) {
      const selectedAffirmation = selectTargetedAffirmation(currentMood);
      setTargetedMoodAffirmation(selectedAffirmation.text);
    } else {
      setTargetedMoodAffirmation(null); // Clear affirmation if mood is reset
    }
  }, [currentMood]); // This effect runs only when 'currentMood' changes
  ...
  const addMoodEntry = () => {
    ...
    // Reset form and targeted affirmation
    setMoodNote('');
    setCurrentMood(null); // This will trigger the useEffect to clear targetedMoodAffirmation
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4"> {/* Added some padding to the main container */}
      <div className="mb-8 text-center">
        <h1 className="text-2xl md:text-3xl font-bold text-purple-800 flex items-center justify-center">
          <Moon className="w-6 h-6 mr-2" />
          Mood Tracker
      ...
      {/* Daily Affirmation (randomly selected once on mount) */}
      {dailyAffirmation && (
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl p-6 mb-8 text-center">
          <h2 className="text-sm uppercase tracking-wider text-purple-600 mb-2">Daily Affirmation</h2>
          <p className="text-xl font-medium text-purple-800">"{dailyAffirmation.text}"</p>
        ...
        {currentMood !== null && (
          <>
            ...
            <div className="flex justify-between items-center">
              <button
                onClick={() => {
                  setCurrentMood(null); // Reset mood selection
                  setMoodNote(''); // Also clear notes on reset
                }}
                className="text-gray-500 hover:text-gray-700"
              >
                Reset
            ...
            {/* Targeted affirmation based on selected mood - now stable! */}
